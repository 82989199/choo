...
public class HellController extends Thread {
		...
		try {
			while (true) {
				...
				L1World.getInstance().broadcastPacketToAll(
						new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "지옥이 열렸습니다. 오픈후부터 1시간동안 입장가능합니다."));
				...
				for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
					if (pc.getMapId() == 6661) {
						// L1Teleport.teleport(pc, 33970, 33246, (short) 4,
						// pc.getMoveState().getHeading(), true);
						pc.start_teleport(33970, 33246, 4, pc.getHeading(), 169, true, false);
						setHellStart(false);
					}
				}
	...
	private void Boss() {
		L1SpawnUtil.spawn2(32807, 32731, (short) 6661, 40173, 0, 600 * 1000, 0);
		L1World.getInstance().broadcastServerMessage("\\aD지옥을 지배하는 구미호가 지옥에 나타났습니다.");
		L1World.getInstance()
				.broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "지옥을 지배하는 구미호가 지옥에 나타났습니다.."));
	}
	/**
	 * 영토가 열려있는지 확인
	 *
...
/*	  private boolean isOpen() { Calendar calender = Calendar.getInstance();
	  int hour, minute;
	  hour = calender.get(Calendar.HOUR_OF_DAY); 
	  minute = calender.get(Calendar.MINUTE); 
	  
	  if ((hour == 03 && minute == 00) || (hour== 11 && minute == 00) || (hour == 19 && minute == 00)) {
		  return true;
		  }
	  return false;
	  }*/
	...
	public boolean isOpen7() {
		NowTime = getTime();
		if ((Integer.parseInt(NowTime) % HELLTIME) >= 2 && (Integer.parseInt(NowTime) % HELLTIME) <= 8)
			return true;
		return false;
	}
	...
	private void close() {
		for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
			if (pc.getMap().getId() == 6661 && pc.isDead()) {
	...
	public void TelePort5() {
		for (L1PcInstance c : L1World.getInstance().getAllPlayers()) {
			...
			case 6661:
				...
				c.start_teleport(33970, 33246, 4, c.getHeading(), 169, true, false);
				break;
			default:
				break;
			}
