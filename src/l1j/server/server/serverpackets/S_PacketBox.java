...
public class S_PacketBox extends ServerBasePacket {
	 ...
	 * [CALL] 버튼이 붙은 윈도우가 표시된다.이것은 BOT등의 부정자 체크에 사용되는 기능한 것같다.이름을 더블 클릭 하면(자)
	 * C_RequestWho가 날아, 클라이언트의 폴더에 bot_list.txt가 생성된다.이름을 선택해+키를 누르면(자) 새로운
	 * 윈도우가 열린다.
	 */
	...
	//TODO 2칸공격 이미지 나올시 추가해야한다
	private static final int spearGfxIds[] = { 1002, 6830, 6831, 7143, 7144, 7147, 7148, 7151, 7152, 7155, 7156, 7159,
			7160, 7163, 7164, // 용기사 샤르나
			11330, 11351, 11344, 11368, 12240, 11447, 14928, 13389, 12237, 11419, 11418, 14923, 13715, 13717, 13719, 15115,
			13721, 13723, 13725, 13727, 13729, 13731, 13733, 13735, 13737, 13739, 13741, 13743, 13745, // 랭킹변신.
			13380, 13381, 14487, // PC꼬꼬마변신, 오렌지
			11121,11125, // 조우변신
			16008,16056, //신규변신
			15115, //남기사 창
//			12541, 12542,// 전사이미지
			...
			11685,12015,
			17545,17549
			,14491,18265,18291,18304,18308,18384,18388,18392
			,18396,18400,18404,18408,18412,18416,18420,18436,18448
			,18452,18560,18564,18568,18572,18576,18580,5146,5150,17288,17411,17415,17439,17575
			};
...
//		9:arrow, 10:gauntlet, 11:claw, 12:edoryu, 13:singlebow, 14:singlespear, 15:tohandblunt, 16:tohandstaff 17:keyring, 18:chainsword
		int range = 1;
		...
		if (weapon != null && weapon.isEquipped()) {
			...
			switch (weaponType) {
			case 1 : case 3 : equipType = 1; break; //한손,양손검
			case 6 : case 15 : equipType = 2; break; //한손도끼,양손도끼
			case 4 : case 13 : case 10 : equipType = 3; range = 15;
				if (weaponType == 4) range = 17; // 양손활일때
				break; // 양손활,한손활,건틀렛
			case 12 : equipType = 4; break; // 이도류
			case 5 : case 14 : case 18 : equipType = 5; range = 2; // 양손창, 한손창, 체인소드
				...
				if (!gfxCk && charBaseGfxId != charGfxId) range = 1;
				break;
			case 7 : case 16 : equipType = 6; break; // 한손지팡이, 양손지팡이
			case 2 : equipType = 7; break; // 단검
			case 11 : case 17 : equipType = 8; break; // 크로우, 키링크
			default: // 그밖에 무기들 없다.
				break;
			}		
		}
		...
		/*if (pc.isGm()) {
			pc.sendPackets(new S_SystemMessage("\\aA공격가능거리 : " + range + " / 변신코드 : " + charGfxId));
		}*/
	}
	...
	public S_PacketBox(int subCode, int time, int gfxid, int type) {
		...
		case EMERALD_ICON:
			...
			writeC(1);//타입
			switch (time) {
			// type 1 - 20프로 / 2-25프로 / 3-30프로 / 4-35프로 / 5-40프로
			case 62: // 성장의 물약들
				writeH(gfxid);
				writeC(20);
				writeC(type);
	...
	public S_PacketBox(int subCode, L1ItemInstance item) {
		...
		case DragonMenu:
			...
			writeC(0);
			break;
		case ITEM_ENCHANT_UPDATE: //1102업뎃이후제작패킷추가
			writeD(item.getId());
			writeC(24);
			writeC(0);
			writeH(0);
			...
			writeC(0);
			writeC(item.getAttrEnchantBit(item.getAttrEnchantLevel()));//제작1102추가
			writeH(0);
			break;
		default:
			break;
