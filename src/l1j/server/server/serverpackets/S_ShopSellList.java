...
public class S_ShopSellList extends ServerBasePacket {
	...
	public S_ShopSellList(int objId, L1PcInstance pc) {
		...
		for (int i = 0; i < shopItems.size(); i++) {
			...
			/**일일 구매제한*/
			int Limitcount = LimitShopController.getInstance().getLimitShop(npcId);
			if(Limitcount > 0){
				for(ShopLimitItem _ShopLimitItem : pc.getLimitShop()){
					if(_ShopLimitItem._npcid == npcId && _ShopLimitItem._itemid == item.getItemId()){
