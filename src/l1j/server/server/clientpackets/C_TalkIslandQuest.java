...
public class C_TalkIslandQuest extends ClientBasePacket {
	...
	private static final int subQuestIds[] = { 309,361,364,365,366,367 };// 미구현 완료시키기.

	public C_TalkIslandQuest(byte[] decrypt, GameClient client) throws IOException {
		super(decrypt);
		...
		case 0x0208: { // 신규 퀘스트 등록
			...
			//synchronized (pc.syncTalkIsland) {
				...
				if (nextid == 259) { // 랜턴
					// 생성
					pc.getInventory().TalkIsland_storeItem(7005, 1);
				}
				
				if (nextid == 260) { // 수련자 변신 주문서
					// 생성
					pc.getInventory().TalkIsland_storeItem(40096, 5);
				}
				
				if (nextid == 262) { // 수련자의 갑옷 마법 주문서
					// 생성
					pc.getInventory().TalkIsland_storeItem(30027, 1);
				}
				
				if (nextid == 264) { // 수련자의 무기 마법 주문서
					// 생성
					pc.getInventory().TalkIsland_storeItem(30028, 1);
				}
				
				if (nextid == 274) { // 용해제
					// 생성
					pc.getInventory().TalkIsland_storeItem(20089, 1);
					pc.getInventory().TalkIsland_storeItem(41245, 10);
				}
				
				if (nextid == 275) { // 마법인형 소환
					// 생성
					pc.getInventory().TalkIsland_storeItem(42654, 1);
					pc.getInventory().TalkIsland_storeItem(41246, 50);
				}
				
				if (nextid == 276) { // 응축된 화염의 기운
					// 생성
					pc.getInventory().TalkIsland_storeItem(6006, 1);
				}
				
				if (nextid == 282) { // 수련자의 장신구 마법 주문서
					// 생성
					pc.getInventory().TalkIsland_storeItem(7004, 1);
					pc.getInventory().TalkIsland_storeItem(22338, 1);
				}
...
//					pc.sendPackets("정식오픈후 다시 한번 수련자의 속도향상 물약을 클릭하여,퀘스트를 진행하세요");
				...
				if (questid == 301) {
					pc.start_teleport(32562, 33089, 9, 5, 169, true, false);
					pc.sendPackets("네르바에게 대화를 걸어 '엘릭서의 룬' 제작 하세요.");
				}
	...
	private void quest_storeitem(L1PcInstance pc, int questid, int type) {
		try {
			...
			if (QuestInfoTable.getInstance().getQuestViewid(questid)) {
				...
				for (int i = 0; i < 4; i++) {
					if (view.item_id[i] != 0) {
						pc.getInventory().TalkIsland_storeItem(view.item_id[i], view.item_count[i]);
					}
				}
				
				if (type != 0) {
					pc.getInventory().TalkIsland_storeItem(42649 + type, 1);
				} else if (type != 0 && questid == 303) { // 전사의 환영물약상자
					pc.getInventory().TalkIsland_storeItem(42655, 1);
				}
