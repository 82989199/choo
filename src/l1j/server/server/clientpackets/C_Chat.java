...
public class C_Chat extends ClientBasePacket {
	...
	private void chatWorld(L1PcInstance pc, String chatText) {
		if (pc.isGm() || pc.getLevel() >= Config.GLOBAL_CHAT_LEVEL) {
			if (pc.isGm() || L1World.getInstance().isWorldChatElabled()) {
				if (pc.get_food() >= 12) {
					...
					if (pc.isGm()) {
						...
						L1World.getInstance().broadcastPacketToAll(new S_NewChat(pc, 4, 3, "\\aD" + chatText, "[******] "));
						return;
					}
					...
					for (L1PcInstance listner : L1World.getInstance().getAllPlayers()) {
						...
						if (!spamList15.contains(0, pc.getName())) {
							...
							S_NewChat cp2 = new S_NewChat(pc, 4, 3, chatText, "");// 장사전챗추가
							listner.sendPackets(cp, true);
							listner.sendPackets(cp2, true);// 장사전챗추가
						}
					}
				} else {
