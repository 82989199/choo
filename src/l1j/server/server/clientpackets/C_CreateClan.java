...
public class C_CreateClan extends ClientBasePacket {
	...
	public C_CreateClan(byte abyte0[], GameClient clientthread) throws Exception {
		...
		if (l1pcinstance.isCrown()) { // 프린스 또는 프린세스
			if (l1pcinstance.getClanid() == 0 && l1pcinstance.getLevel() >= Config.Crown_Level) {
				...
				l1pcinstance.getInventory().consumeItem(L1ItemId.ADENA, 10000);
				if (clan != null) {
					...
					l1pcinstance.start_teleport(l1pcinstance.getX(), l1pcinstance.getY(), l1pcinstance.getMapId(), l1pcinstance.getHeading(), 169, false, false);
				}
				...
				l1pcinstance.sendPackets(new S_SystemMessage("이미 혈맹이 있거나 "+Config.Crown_Level+"레벨 이상만 창설할 수 있습니다.")); // 추가
			}
		} else {
