...
public class C_BoardWrite extends ClientBasePacket {
	...
	public C_BoardWrite(byte decrypt[], GameClient client) {
		...
		if (tg instanceof L1BoardInstance) {
			...
			if (board != null) {
				if (pc.getLevel() < Config.BoardLevel && board.getNpcId() != 900006 && board.getNpcId() != 80006 && board.getNpcId() != 500002) {
					pc.sendPackets(new S_SystemMessage("레벨 "+Config.BoardLevel+" 아니시거나, GM게시판 입니다."));
					return;
				}
			}
			...
			case 4200015: // GM 서버정보게시판 board_posts_notice
				if (pc.getAccessLevel() == Config.GMCODE) {
					...
					pc.sendPackets(new S_SystemMessage( "해당 게시판은 운영자 전용 게시판 입니다."));
					return;
				}
				break;
			...
			case 500002: // 건의 사항^건의 게시판 board_posts_fix
				if (pc.getInventory().checkItem(L1ItemId.ADENA, 500)) {
					...
					pc.sendPackets(new S_SystemMessage("등록 완료 : 글 제목은 운영자도 못보니 참고하세요."));
				} else {
