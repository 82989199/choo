...
public class C_CommonClick {
	...
	public C_CommonClick(GameClient client) {
		...
		if(Config.CHAR_PASSWORD && (client.getAccount().getCPW() == null || client.getAccount().getCPW().equalsIgnoreCase("")))		
			client.sendPacket(new S_CharPass(S_CharPass._케릭선택창진입3));
		
		client.sendPacket(new S_CharAmount(amountOfChars, slot));
		client.sendPacket(new S_CharPass(S_CharPass._케릭선택창진입2));
		if (amountOfChars > 0) {
			sendCharPacks(client);
		}
		client.sendPacket(new S_CharPass(S_CharPass._케릭선택창진입));
