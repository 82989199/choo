	...
	public void onTalkAction(L1PcInstance player) {
		...
		if (talking != null) {
			...
			/*case 81210:
				...
				break;*/
			case 7320101: // 초보자 도우미 카시오페아 (붉은 기사단 훈련소)
				player.sendPackets(new S_SystemMessage( "\\f2OO서버에 오신 것을 환영합니다."));
				player.start_teleport(33439, 32812, 4, player.getHeading(), 169, true, false);
				break;
				
			case 8500299: // 투견장 텔레포터 제티 (기란 십자가 -> 투견장/버경장)
				player.sendPackets(new S_SystemMessage( "\\f2투견장에 오신 것을 환영합니다."));
				player.start_teleport(33540, 32850, 4, player.getHeading(), 169, true, false);
				break;
				
			case 7320215:
				if (player.getLevel() >= 1 & player.getLevel() <= 55) {
					if (player.getLevel() < Config.경험치지급단) {
						player.addExpForReady((ExpTable.getExpByLevel(Config.경험치지급단) - 1) - player.getExp()+ ((ExpTable.getExpByLevel(Config.경험치지급단) - 1) / 1000));
			...
			case 202057://라라
				if (player.getLevel() >= 56) {
					htmlid = "tel_lala2";
				} else if (player.getLevel() >= 54 && player.getLevel() <= 55) {
					player.sendPackets(new S_SystemMessage( "게렝에게 군터의 추천서를 주고 버프를 받으세요."));
					player.start_teleport(32586, 32954, 0, player.getHeading(), 169, true, false);
					htmlid = "tel_lala2";
				}
				break;
			case 70841:
			...
			case 522:
				if (player.getInventory().checkItem(40308, 200000)) {
					...
					if (Config.POLY_EVENT1) {
						L1PolyMorph.doPoly(player, 7925, 600, L1PolyMorph.MORPH_BY_ITEMMAGIC, false);
					}
			...
			case 70514:
			 ...
			 /* case 70035: case 70041: // 버경 상인 case 70042:
			 * if(BugRaceController.getInstance().getBugState() == 1){ htmlid =
			 * "maeno3"; }else if(BugRaceController.getInstance().getBugState()
			 * == 2){ htmlid = "maeno5"; }else{ htmlid = "pandora"; } break;
			 */
			case 70035:
			case 70041:
			case 70042:
...
//				}
			...
			case 7200000:
				if (player.getLevel() >= 56) {
					if (player.getInventory().checkItem(3000215, 1) || player.getInventory().checkItem(1000004, 1)) {
						htmlid = "ekins2";
					} else {
						htmlid = "ekins1";
					}
			...
			/*	if (!player.getQuest().isEnd(L1Quest.QUEST_HIDDENVALLEY) && player.getLevel() > 9) {
					...
					createNewItem(player, npcName, L1ItemId.IVORYTOWER_WEAPON_SCROLL, 1, 0); // 상아탑의
					createNewItem(player, npcName, L1ItemId.IVORYTOWER_ARMOR_SCROLL, 4, 0); // 상아탑의
					htmlid = "tutorrw9";
				}*/
				int[] Buff = new int[] { HASTE, FULL_HEAL };
				player.setBuffnoch(1);
				for (int i = 0; i < Buff.length; i++) {
					l1skilluse.handleCommands(player, Buff[i], player.getId(), player.getX(), player.getY(), null, 0,
				...
				player.setCurrentMp(player.getMaxMp());
				break;
		/*	case 61841: //클라우디아 마을 라라
				int[] Buff1 = new int[] { HASTE, FULL_HEAL };
				player.setBuffnoch(1);
				for (int i = 0; i < Buff1.length; i++) {
					l1skilluse.handleCommands(player, Buff1[i], player.getId(), player.getX(), player.getY(), null, 0,
							L1SkillUse.TYPE_GMBUFF);
				}
				player.setBuffnoch(0);
				player.setCurrentHp(player.getMaxHp());
	...
	private void 경험치멘트(L1PcInstance pc) {
		 ...
		 * "시스템: [경험치 지급은 Lv.("+ Config.경험치지급단 +") 까지 가능합니다]"));
		 * pc.sendPackets(new S_SystemMessage("\\aA시스템: 경험치 지급은 \\aG[" +
		 * Config.경험치지급단 + "]\\aA 까지 가능합니다"));
		 */
	}
