...
import l1j.server.server.serverpackets.S_EtcPacket;
	...
	*/
	...
	public void removeRetiredMembers() {
		...
		for (L1PcInstance pc : L1World.getInstance().getAllPlayers3())
		{
			...
			{
				pc.start_teleport(32616 + Rnd(4), 32774 + Rnd(4), 4, 5, 169, true, false);
			}
		}
	}
...
//						L1Teleport.teleport(pc, 32616 + Rnd(4), 32774 + Rnd(4), (short)4, 5, true);
						pc.start_teleport(32616 + Rnd(4), 32774 + Rnd(4), 4, 5, 169, true, false);
					}
					removeMember(pc);
		...
		for(L1PcInstance pc : toArray(일반)){
			L1PolyMorph.doPoly(pc, 5065, 1000,L1PolyMorph.MORPH_BY_NPC, false); //아기 진돗
	...
	private void openDoor()
	{
		...
		for (L1Object object : L1World.getInstance().getObject()) {
			if (object instanceof L1DoorInstance) {
				...
				if (door.getCurrentSpriteId() == 6677){
					door.open();
				}
			}
		}
	...
	private void closeDoor()
	{
		...
		for (L1Object object : L1World.getInstance().getObject()) {
			if (object instanceof L1DoorInstance) {
				...
				if (door.equalsCurrentSprite(6677)){
					door.close();
				}
			}
		}
		...
		for(int i = 0; i < 18; i++)
		{
			for (L1PcInstance pc : toArray(i)) {
				if (pc.getMapId() == getMapId()) {
					...
					pc.start_teleport(32624, 32813, 4, 5, 169, true, false);
				}
			}
		}
