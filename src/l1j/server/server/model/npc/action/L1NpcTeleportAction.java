...
public class L1NpcTeleportAction extends L1NpcXmlAction {
	...
	public L1NpcHtml execute(String actionName, L1PcInstance pc, L1Object obj, byte[] args) {
		...
		if(_loc.getMapId() == 622){
			if (!pc.getInventory().checkItem(41921, 2)) {
				pc.sendPackets(new S_ServerMessage(337, "$19487"));
				return L1NpcHtml.HTML_CLOSE;
			}
		}else {
		...
		if(_loc.getMapId() == 622){
			pc.getInventory().consumeItem(41921, _price);
		}else {
			pc.getInventory().consumeItem(L1ItemId.ADENA, _price);
		}
			//pc.sendPackets(new S_Teleport(pc));
//			L1Teleport.teleport(pc, location.getX(), location.getY(), (short) location.getMapId(), _heading, _effect);
			pc.start_teleport(location.getX(), location.getY(), location.getMapId(), _heading, 169, _effect, false);
			//return L1NpcHtml.HTML_CLOSE;
