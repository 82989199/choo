		...
		try {
			...
			while (rs.next()) {
				try {
					...
					int count = rs.getInt("count");
					int equipitem = rs.getInt("item_id");
					if (count <= 0) continue;
					...
					//TODO 수련자의 장비류 자동착용(추후에있을시 번호추가)
					if (equipitem >= 22300 && equipitem <= 22311
							|| equipitem == 48 || equipitem == 120 || equipitem == 147 
							|| equipitem == 156 || equipitem == 175) {
						pstm2.setInt(6, 1);
					} else {
						pstm2.setInt(6, 0);
					}
