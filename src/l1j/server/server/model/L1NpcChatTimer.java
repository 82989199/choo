...
public class L1NpcChatTimer implements Runnable {
	...
	private void chat(L1NpcInstance npc, int chatTiming, String chatId, 
		...
		if (!isShout) {
//			npc.broadcastPacket(new S_NpcChatPacket(npc, chatId, 0));  //-- 채창 npc글씨 보이게 할꺼면 이걸로 바꾸기
			npc.broadcastPacket(new S_NpcChatPacket(npc, chatId, 4)); //-- 채창 npc글씨 안보이게 할꺼면 이걸로 바꾸기
		} else {
