...
public class L1Search implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			while (st.hasMoreTokens()) {
				...
				if (itCount == 0
						&& (tempVar.equals("갑옷") || tempVar.equals("무기")
								|| tempVar.equals("etc")
								|| tempVar.equals("변신") || tempVar
									.equals("NPC"))) {
					simpleS = false;
					type = tempVar;
				} else {
			...
			pc.sendPackets(new S_SystemMessage(
					".find [갑옷,무기,etc,변신,NPC] 를 입력 해주세요"));
		}
	}
	...
	private void find_object(L1PcInstance pc, String type, String name) {
		...
		try {
			...
			if (error == false) {
				...
				pc.sendPackets(new S_SystemMessage("라인에 검색 하는 '"
						+ name.replace("%", " ") + " ' 는" + type + "이름..."));
				while (rs.next()) {
					str1 = rs.getString(1);
					str2 = rs.getString(2);
					if (type.equals("갑옷") || type.equals("무기")
							|| type.equals("etc")) {
						bless = rs.getInt(3);
						if (bless == 1) {
							blessed = "";
						...
						pc.sendPackets(new S_SystemMessage(blessed + "번호: "
								+ str1 + ", " + str2));
					} else {
						pc.sendPackets(new S_SystemMessage("번호: " + str1 + ", "
								+ str2));
					}
					count++;
				}
				pc.sendPackets(new S_SystemMessage("찾기 " + count + " 라인에 항목"
						+ type + "타입。"));
			}
		} catch (Exception e) {
		} finally {
	...
	private void find_object(L1PcInstance pc, String name) {
		...
		try {
			...
			pc.sendPackets(new S_SystemMessage(" "));
			pc.sendPackets(new S_SystemMessage("라인에서 검색 한 개체 이름: '"
					+ name.replace("%", " ") + "'"));
			...
			while (rs.next()) {
				...
				pc.sendPackets(new S_SystemMessage(blessed + "번호: " + str1
						+ ", " + str2));
				count1++;
			}
		} catch (Exception e) {
