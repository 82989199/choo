...
public class NpcTable {
	...
	private final HashMap<Integer, L1Npc> _npcs = new HashMap<Integer, L1Npc>(4096);
	...
	private void loadNpcBorn(){
		...
		try{
			...
			while(rs.next()){
				...
				if(npc == null){
					System.out.println(String.format("[npc_born load fail.]엔피씨아이디를 찾을 수 없습니다. %d", npcid));
					continue;
				}				
				npc.createBorn(rs);
			}
	...
	private void loadNpcData() {
		...
		try {
			...
			while (rs.next()) {
				...
				npc.set_undead(rs.getInt("undead")); //0일반/1언데드/2그외
				...
				npc.setShapeChange(rs.getBoolean("shape_change"));
				_npcs.put(npcId, npc);
			}
