...
public class FantasyIsland implements Runnable {
	...
	private int[] bossInt = {14905, 14907, 7200020};
	private int _status;
			...
			try {
				...
				if (unicorn.isDead()) {
					if (pc != null) {
						...
						pc.start_teleport(33968, 32961, 4, 2, 169, true, false);
				...
				case WAIT_RAID:
					...
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "$17947"));//마법 막대를 이용해 적을 처치해주세요.
					pc.getInventory().storeItem(810006, 1);
					pc.sendPackets(new S_SystemMessage("$17948"));
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "경고! 오토방지 코드는 무시하세요."));
					
					Sleep(3000);
					pc.sendPackets(new S_SystemMessage( "\\aA팁: 오토방지 코드는 무시하시면 됩니다.(\\aG자동인증\\aA)"));
					
					Sleep(5000);
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "$17701"));
					pc.sendPackets(new S_PacketBox(S_PacketBox.ROUND, 1, 4));
					//적들이 몰려오고 있습니다.
					//11시 방향 스폰 포르시스, 베네보스
				...
				case SECOND_STEP:
					Sleep(10000);
					pc.sendPackets(new S_PacketBox(S_PacketBox.ROUND, 2, 4));
					...
					Sleep(5000);
					//11시 포르시스, 베네보스
					L1SpawnUtil.spawnCount(32799, 32852, _map, 7200008, 0, 0, 7);
					L1SpawnUtil.spawnCount(32799, 32852, _map, 7200012, 0, 0, 7);
					//1시방향 메두사+스콜피온 + 땅의대정령
					L1SpawnUtil.spawnCount(32810, 32863, _map, 7200009, 0, 0, 7);
					L1SpawnUtil.spawnCount(32810, 32863, _map, 7200013, 0, 0, 7);
					L1SpawnUtil.spawnCount(32810, 32863, _map, 7200018, 0, 0, 1);
					Sleep(10000);
					//5시 코트룻+페르페르
					L1SpawnUtil.spawnCount(32801, 32873, _map, 7200010, 0, 0, 7);
					L1SpawnUtil.spawnCount(32801, 32873, _map, 7200014, 0, 0, 7);
					Sleep(20000);
					//7시 메가+비아
					L1SpawnUtil.spawnCount(32789, 32861, _map, 7200011, 0, 0, 7);
					L1SpawnUtil.spawnCount(32789, 32861, _map, 7200015, 0, 0, 7);
					Sleep(5000);
					//11시 포르시스, 베네보스
					L1SpawnUtil.spawnCount(32799, 32852, _map, 7200008, 0, 0, 7);
					L1SpawnUtil.spawnCount(32799, 32852, _map, 7200012, 0, 0, 7);
				...
				case THIRD_STEP:
					Sleep(3000);
					pc.sendPackets(new S_PacketBox(S_PacketBox.ROUND, 3, 4));
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "$17703"));
					//적들이 더 몰려옵니다. 준비해 주세요
					pc.getInventory().storeItem(810006, 1);
					pc.sendPackets(new S_SystemMessage("$17948"));
					...
					L1SpawnUtil.spawnCount(32799, 32852, _map, 7200012, 0, 0, 7);
					Sleep(5000);
					//1시 메두사+스콜피온, 5시 코트룻+페르페르 스폰, 바람의대정령
					L1SpawnUtil.spawnCount(32810, 32863, _map, 7200009, 0, 0, 7);
					L1SpawnUtil.spawnCount(32810, 32863, _map, 7200013, 0, 0, 7);
					L1SpawnUtil.spawnCount(32801, 32873, _map, 7200010, 0, 0, 7);
					L1SpawnUtil.spawnCount(32801, 32873, _map, 7200014, 0, 0, 7);
				...
				case FOURTH_STEP:
					Sleep(3000);
					pc.sendPackets(new S_PacketBox(S_PacketBox.ROUND, 4, 4));
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "$17703"));
					//적들이 더 몰려옵니다. 준비해 주세요
					pc.getInventory().storeItem(810006, 1);
					pc.sendPackets(new S_SystemMessage("$17948"));
					Broadcaster.broadcastPacket(unicorn, new S_NpcChatPacket(unicorn, "$17706", 0));
					//얼마 남지 않았습니다. 조금만 더 버텨주세요
					...
					if(chance <= 30) {
						boss = L1SpawnUtil.spawnCount(32789, 32861, _map, bossInt[0], 0, 0, 1);
					}else if(chance > 30 && chance <= 70) {
						boss = L1SpawnUtil.spawnCount(32789, 32861, _map, bossInt[1], 0, 0, 1);
					}else {
						boss = L1SpawnUtil.spawnCount(32789, 32861, _map, bossInt[2], 0, 0, 1);
					}
					stage = 6;
					break;
				case LAST_STEP:
					if (boss.isDead() || boss == null) {
						Sleep(5000);
//						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "$17707"));////몽환의 지배자가 퇴치 되었습니다.
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "지배자가 퇴치 되었습니다."));
						Sleep(5000);
						Broadcaster.broadcastPacket(unicorn, new S_SkillSound(unicorn.getId(), 1911));
						Sleep(1000);
						unicorn.sendShape(12493);
						//Sleep(5000);
						//감사합니다!
						...
						L1ItemInstance item = ItemTable.getInstance().createItem(810010);
				...
				case END:
					...
					//						//시스템 메시지: 10초 후에 마을로 텔레포트 합니다.
					//					}
					Sleep(5000);
					if (pc.getMapId() == _map) {
						pc.sendPackets(new S_ServerMessage(1480));
						//시스템 메시지: 5초 후에 마을로 텔레포트 합니다.
					}
					//					Sleep(1000);
					//					if (pc.getMapId() == _map) {
					//						pc.sendPackets(new S_ServerMessage(1481));
					//						//시스템 메시지: 4초 후에 마을로 텔레포트 합니다.
					//					}
					//					Sleep(1000);
					//					if (pc.getMapId() == _map) {
					//						pc.sendPackets(new S_ServerMessage(1482));
					//						//시스템 메시지: 3초 후에 마을로 텔레포트 합니다.
					//					}
					//					Sleep(1000);
					//					if (pc.getMapId() == _map) {
					//						pc.sendPackets(new S_ServerMessage(1483));
					//						//시스템 메시지: 2초 후에 마을로 텔레포트 합니다.
					//					}
					//					Sleep(1000);
					//					if (pc.getMapId() == _map) {
					//						pc.sendPackets(new S_ServerMessage(1484));
					//						//시스템 메시지: 1초 후에 마을로 텔레포트 합니다.
					//					}
					pc.start_teleport(33437, 32813, 4, 2, 169, true, false);
					pc.getInventory().consumeItem(810006);
					pc.getInventory().consumeItem(810007);
					pc = null;
					break;
	...
	public void Start(){
		...
		System.out.println(""+ 오전오후 + " " + cal.get(시간) + "시" + cal.get(분) + "분" + "   ■중앙사원 시작■캐릭터:"+pc.getName()+"■맵:"+_map+"■");
	}
	private void endRaid(){
		...
		if (Running) {
			...
			System.out.println(""+ 오전오후 + " " + cal.get(시간) + "시" + cal.get(분) + "분" + "   ■중앙사원 종료■캐릭터:"+pc.getName()+"■맵:"+_map+"■");
		}
	}
}
